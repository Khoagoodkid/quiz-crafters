...
const TopBar = ({ isOpenSetting, setIsOpenSetting }) => {
  return (
    <section
    ...
    >
      <GlassNavigation isOpenSetting={isOpenSetting} setIsOpenSetting={setIsOpenSetting} />
...
const GlassNavigation = ({ setIsOpenSetting }) => {
  ...
  return (
    ...
    >
      <div className="glass-nav flex items-center justify-between px-5 py-5 max-h-28">
        <Cursor hovered={hovered} scope={scope} />

        <Links setIsOpenSetting={setIsOpenSetting} />
...
const Logo = () => (
  <img src="/logo.png"
    className="w-40 h-4/5 " />


);

const Links = ({ setIsOpenSetting }) => {
  const navigate = useNavigate()
  return (
    <div className="hidden items-center gap-2 md:flex">
      <GlassLink text="Sign Up" onClickFunc={() => navigate('/signup')} />
      <GlassLink text="Setting" onClickFunc={() => setIsOpenSetting(true)} />
...
const GlassLink = ({ text, onClickFunc }) => {
...
const SignInButton = () => {
  const navigate = useNavigate()
  return (
    <button onClick={() => navigate('/login')} className="relative scale-100 overflow-hidden rounded-lg bg-gradient-to-br from-indigo-600 from-40% to-indigo-400 px-4 py-2 font-medium text-white transition-transform hover:scale-105 active:scale-95">
      <span className="relative z-10 text-white/90 transition-colors group-hover:text-white">
        Sign in
      </span>
      <span className="absolute inset-0 z-0 bg-gradient-to-br from-white/20 to-white/5 opacity-0 transition-opacity group-hover:opacity-100" />
